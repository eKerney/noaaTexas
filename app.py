...
def getDailyNormalsData(noaa, m, y, s):
    ...
    noaa.stationDataParams('NORMAL_DLY', (f'GHCND:{sta[s]}'), (f'{2010}-{mon[m]}-{day[m][0:2]}') , (f'{2010}-{mon[m]}-{day[m][3:5]}'), 1000, 'standard',
        ['DLY-DUTR-NORMAL','DLY-PRCP-25PCTL','DLY-PRCP-50PCTL','DLY-PRCP-75PCTL','DLY-PRCP-PCTALL-GE001HI','DLY-PRCP-PCTALL-GE010HI',
...
def dailyNormalsPlots(df, station, year, month):
    ...
    line1 = ax.plot(ind+width, df['DLY-PRCP-25PCTL'], color = AWNDc, linewidth=3.0, alpha=0.7)
    line2 = ax.plot(ind+width, df['DLY-PRCP-50PCTL'], color = AWNDc, linewidth=3.0, alpha=0.7)
    line3 = ax.plot(ind+width, df['DLY-PRCP-75PCTL'], color = AWNDc, linewidth=3.0, alpha=0.7)
    ...
    width = 0.4
    Pc, Pe, Sc, Se = '#006be6','#001a38','#5d6875','#22262b'
    line1 = ax.plot(ind+width, df['DLY-PRCP-PCTALL-GE001HI'], color = AWNDc, linewidth=3.0, alpha=0.7)
    line2 = ax.plot(ind+width, df['DLY-PRCP-PCTALL-GE010HI'], color = AWNDc, linewidth=3.0, alpha=0.7)
    line3 = ax.plot(ind+width, df['DLY-PRCP-PCTALL-GE050HI'], color = AWNDc, linewidth=3.0, alpha=0.7)
    line4 = ax.plot(ind+width, df['DLY-PRCP-PCTALL-GE100HI'], color = AWNDc, linewidth=3.0, alpha=0.7)
    plt.ylabel('%', fontsize=12)
    plt.xticks(ind+width/2,df['dayYear'])
    legend_elements = [Patch(facecolor=Pc, edgecolor=Pe, label='Hourly Overcast %', alpha=0.8),
        Patch(facecolor=Sc, edgecolor=Se, label='Hourly Clear %',alpha=0.7)]
